		...
		resourceApp.controller('customerrequestedresourceCtrl',["$scope","$rootScope","$state","$http","$stateParams","RAService",function($scope,$rootScope, $state,$http, $stateParams, RAService){
			...
			$scope.getresourcelist = function(){
				debugger;
				$scope.local = localStorage.getItem('registrationId');
				RAService.customerrequestedresource($scope.local).then(function(data) {
			        $scope.resourcelist = data.result;
			        console.log($scope.resourcelist);
			        $scope.totalCount = data.count;
			        $scope.skillslist = data.allList;
